...
private class CompilationEnvironment(
    ...
    init {
        environment = KotlinCoreEnvironment.createForProduction(
            ...
            configuration = KotlinCompilerConfiguration().apply {
                ...
                if (scriptsConfig.enabled) {
                    ...
                    val foundDSLDependency =
                        classPath.any { GRADLE_DSL_DEPENDENCY_PATTERN.matches(it.fileName.toString()) }
                                ...
                                ScriptDefinition.FromLegacy(
                                    ...
                                    object : KotlinScriptDefinitionFromAnnotatedTemplate(
                                        ...
                                        override fun isScript(fileName: String): Boolean {
                                            ...
                                            if (this.template.simpleName == "KotlinSettingsScript" && fileName.endsWith(
                                                    "settings.gradle.kts"
                                                )
                                            ) {
                                                return true
        ...
        scriptDefinitions.takeIf { it.isNotEmpty() }?.let {
            val annotations = scriptDefinitions.flatMap {
                it.asLegacyOrNull<KotlinScriptDefinition>()?.annotationsForSamWithReceivers ?: emptyList()
            }
